export default function SkeletonLoader({ type = "generic", count = 3 }) {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className="p-5 rounded-2xl border border-white/5 bg-white/2 backdrop-blur-md space-y-3"
          >
    ...
    return (
      <div className="space-y-2.5 w-full">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className="flex items-center justify-between p-3 rounded-xl border border-white/5 bg-white/1"
          >
  ...
  if (type === "graph") {
    return (
      <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center border border-white/5 bg-black/20 rounded-2xl p-6 relative overflow-hidden">
        {/* Shimmering nodes representation */}
        ...
        <div className="text-center z-10 space-y-2 animate-pulse">
          ...
          <p className="text-[10px] uppercase font-mono font-bold tracking-widest text-indigo-400">
      ...
      <div className="space-y-4 p-4 w-full">
        {Array.from({ length: count }).map((_, i) => {
          ...
          return (
            ...
            >
              <div
                className={`w-6 h-6 rounded-full shrink-0 ${shimmerClass}`}
              ></div>
              <div className="space-y-2 flex-grow">
                <div
                  className={`h-3 w-24 rounded ${shimmerClass} ${
                    isLeft ? "" : "ml-auto"
                  }`}
                ></div>
                <div
                  className={`p-3 rounded-2xl border border-white/5 space-y-1.5 ${
                    isLeft
                      ? "bg-white/3 rounded-tl-none text-left"
                      : "bg-indigo-600/10 border-indigo-500/10 rounded-tr-none text-right"
                  }`}
                >
