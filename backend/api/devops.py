...
def generate_devops_or_doc_asset(payload: DevOpsGeneratePayload):
    ...
    try:
        ...
        elif a_type in {"dockerfile", "docker-compose", "github-actions", "kubernetes", "helm"}:
            content = generate_devops_asset(payload.repo_path, a_type)
        else:
            raise HTTPException(status_code=400, detail=f"Unsupported asset type: {payload.asset_type}")
