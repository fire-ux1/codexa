...
def get_repo_obj(repo_path: str) -> Repo:
    ...
    if not abs_path.startswith(abs_repos):
        raise PermissionError(
            "Access denied: path is outside the repositories directory"
        )

    if not os.path.isdir(abs_path) or not os.path.exists(
    ...
    try:
        ...
        if file_path:
            ...
            rel_file_path = os.path.relpath(
                os.path.abspath(file_path), repo.working_dir
            )
...
def git_blame(repo_path: str, file_path: str):
    ...
    try:
        ...
        return {"status": "success", "blame": blame_data}
    except Exception as e:
        return {"status": "error", "message": str(e)}
