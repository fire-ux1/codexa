...
from services.knowledge_graph.graph_storage import clear_graph, insert_nodes, insert_edges
...
def parse_db_tables(code: str) -> list[str]:
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, code):
            ...
            if tbl not in {"select", "insert", "update", "delete", "where", "set", "values", "left", "right", "inner", "outer"}:
...
def build_knowledge_graph(repo_path: str, repo_id: str):
            ...
            try:
                ...
                for node in ast.walk(tree):
                    ...
                    # B. Functions (Top-level or inside classes)
                        ...
                        func_display_name = f"{parent_class}.{node.name}" if parent_class else node.name
                        ...
                        for dec in node.decorator_list:
                            ...
                            if isinstance(dec, ast.Call):
                                ...
                                if dec_str.lower() in {"get", "post", "put", "delete", "route", "patch"}:
                                    is_route = True
                                ...
                                if dec.attr.lower() in {"get", "post", "put", "delete", "route", "patch"}:
                                    is_route = True
                        ...
                        if is_route:
                            ...
                            if not any(n["id"] == endpoint_node_id for n in nodes):
                                nodes.append({
                                    ...
                                    "type": "endpoint",
                                    "path": route_path,
                                    "meta": {"methods": methods, "route": route_path}
                                })
                            edges.append({
                                "id": str(uuid.uuid4()),
                                "source": func_node_id,
    ...
    print(f"Knowledge Graph successfully built: {len(nodes)} nodes, {len(edges)} edges saved.")
